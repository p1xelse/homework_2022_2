...
QUnit.module("Тестируем функцию zip", function () {
    ...
    QUnit.test(
        "Функция правильно работает с одинаковыми объектами",
        function (assert) {
            assert.deepEqual(zip({ answer: 42 }, { answer: 42 }), {
                answer: 42,
            });
            assert.deepEqual(zip({ age: 5 }, {}, { age: 5 }, { age: 5 }), {
                age: 5,
            });
            assert.deepEqual(zip({}, {}, {}), {});
        }
    ...
    QUnit.test(
        "Функция бросает исключение, если нет аргументов",
        function (assert) {
    ...
    QUnit.test(
        "Функция бросает исключение, если хотя бы один из аргументов не объект",
        function (assert) {
